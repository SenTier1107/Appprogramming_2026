...
| **11주차**<br>2026.05.15 | [`Wine_Classification_Dashboard`]([./Wine_Classification_Dashboard](https://github.com/SenTier1107/Appprogramming_2026/tree/main/wine_variety_dashboard)) | - scikit-learn `load_wine` 데이터셋을 활용한 RandomForest 분류 모델 학습<br>- Feature Importance 분석 — 누적 중요도 기반 상위 6개 변수 선정 (80.7% 설명력)<br>- FastAPI + Gradio `mount_gradio_app` 단일 서버 구조로 대시보드 구현<br>- 3탭 구성: 전체 데이터 조회 / 피처 중요도 시각화 / 품종 예측<br>- 다크 테마 차트 (개별 중요도 막대 + 누적 중요도 꺾은선 이중 축)<br>- Docker + Hugging Face Spaces 배포 |
